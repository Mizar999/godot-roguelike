...
func _init(map_p, debug_draw_p):
	...
	for cell in map.get_passable_cells(3):
...
func spawn(path: String, cell: Vector2):
	...
	if entity.is_in_group(RL.GROUP_BLOCKER):
		pathfinder.dirty_cells[entity.cell] = false
...
func move(actor, cell: Vector2) -> void:
	if actor.is_in_group(RL.GROUP_BLOCKER):
		pathfinder.dirty_cells[actor.cell] = true
		pathfinder.dirty_cells[cell] = false
	actor.cell = cell
